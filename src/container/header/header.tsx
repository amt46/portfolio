...
const Header = ({ homeRef }) => {
  ...
  const hoverEffect = (e) => {
    var x = e.pageX - e.target.offsetLeft - e.target.offsetParent.offsetLeft;
      ...
      <div className="flex-auto w-64 p-40">
        <div className="h-full rc flex flex-col justify-evenly">
          ...
          >
            <Button
              style={
                windowSize.innerWidth <= 640
                  ? { width: `${img2.current?.offsetWidth}px` }
                  : {}
              }
              className="btn"
            ...
            <a href="#projects" ref={projectsRef} style={{ display: "none" }}>
              none
            </a>
            <a href="#skills" ref={skillsRef} style={{ display: "none" }}>
              none
            </a>
          </motion.div>
        </div>
        ...
        >
          {skills.map((i) => {
            return (
              <motion.div key={i.name} className={`${i.class} sk wpx-70 pa`}>
                <div className="pr w-100 h-100">
                  <img
                    ...
                    onMouseEnter={() => {
                      if (i.name === "Javascript") setIsImg(!isImg);
                    }}
                    src={i.image}
                    alt={i.name}
                  />
